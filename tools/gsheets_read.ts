  ...
  try {
    ...
    if (error.code === 404) {
      return {
        content: [
          {
            type: "text",
            text: `Spreadsheet not found: The spreadsheet with ID '${args.spreadsheetId}' does not exist or you don't have access to it.`,
          },
        ],
        isError: true,
      };
    ...
    if (error.code === 403) {
      return {
        content: [
          {
            type: "text",
            text: "Permission denied: You don't have permission to read this spreadsheet.",
          },
        ],
        isError: true,
      };
    ...
    if (error.code === 400) {
      return {
        content: [
          {
            type: "text",
            text: `Invalid request: ${error.message || "Please check your input parameters."}`,
          },
        ],
        isError: true,
      };
    ...
    return {
      content: [
        {
          type: "text",
          text: `Error reading spreadsheet: ${error.message || "Unknown error occurred"}`,
        },
      ],
      isError: true,
    };
