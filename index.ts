...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  // Import the specific handler function instead of using the generic tools array
  const { readFile } = await import("./tools/gdrive_read_file.js");
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  // Don't require auth for deauthenticate tool
  if (request.params.name !== "gdrive_deauthenticate") {
    await ensureAuth();
  }
